...
@section('content')
    ...
    <p>
        <a href="{{ route('billpayment.billpayment.create') }}" class="btn btn-success">@lang('global.app_add_new')</a>
    </p>
                    ...
                    @if (count($otherBilling) > 0)
                        @foreach ($otherBilling as $data)
                            <tr data-entry-id="{{ $data->id }}">                                
                                ...
                                <td>
                                    <a href="{{ route('billpayment.billpayment.edit',[$data->id]) }}" class="btn btn-xs btn-warning">@lang('global.app_edit')</a>
                                    <a href="{{ route('billpayment.billpayment.show',[$data->id]) }}" class="btn btn-xs btn-info">View</a>
                                    @if ($data->isFile == 1)
                                    ...
                                    {!! Form::open(array(
                                        ...
                                        'route' => ['billpayment.billpayment.destroy', $data->id])) !!}
